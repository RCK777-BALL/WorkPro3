...
export const markNotificationRead = async (req, res, next) => {
    ...
    try {
        const updated = await Notification.findOneAndUpdate({ _id: req.params.id, tenantId: req.tenantId }, { read: true }, { new: true });
        if (!updated)
            return res.status(404).json({ message: 'Not found' });
        res.json(updated);
    }
...
export const updateNotification = async (req, res, next) => {
    ...
    try {
        const updated = await Notification.findOneAndUpdate({ _id: req.params.id, tenantId: req.tenantId }, req.body, {
            new: true,
            runValidators: true,
        });
        if (!updated)
