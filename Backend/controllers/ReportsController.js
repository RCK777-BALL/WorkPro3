...
async function calculateStats(tenantId, role) {
    ...
    const completed = await WorkOrder.find({ completedAt: { $exists: true }, tenantId }, {
        createdAt: 1,
        completedAt: 1,
    });
    const averageResponseTime = completed.length
        ?
            completed.reduce((sum, o) => {
                const diff = (o.completedAt.getTime() - o.createdAt.getTime()) / 36e5; // ms to hours
                return sum + diff;
            }, 0) / completed.length
        : 0;
...
export const exportTrendData = async (req, res, next) => {
    try {
...
async function aggregateCosts(tenantId) {
    ...
    [labor, maintenance, materials].forEach((arr) => {
        arr.forEach((r) => {
            ...
            if (r.laborCost !== undefined)
                entry.laborCost = r.laborCost;
            if (r.maintenanceCost !== undefined)
                entry.maintenanceCost = r.maintenanceCost;
            if (r.materialCost !== undefined)
                entry.materialCost = r.materialCost;
    ...
    return Array.from(map.values())
        ...
        .map((r) => ({
        ...r,
        totalCost: (r.laborCost || 0) +
            (r.maintenanceCost || 0) +
            (r.materialCost || 0),
    }));
}
export const getCostMetrics = async (req, res, next) => {
    try {
        const tenantId = req.tenantId;
...
export const getDowntimeMetrics = async (req, res, next) => {
    try {
        const tenantId = req.tenantId;
