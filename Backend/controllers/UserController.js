...
export const updateUser = async (req, res, next) => {
    try {
        const updated = await User.findOneAndUpdate({ _id: req.params.id, tenantId: req.tenantId }, req.body, {
            new: true,
            runValidators: true,
        }).select('-password');
        if (!updated)
...
export const updateUserTheme = async (req, res, next) => {
    try {
        ...
        const user = await User.findByIdAndUpdate(req.params.id, { theme }, { new: true }).select('theme');
        if (!user)
            return res.status(404).json({ message: 'Not found' });
