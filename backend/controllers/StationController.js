...
export const getAllStations = async (req, res, next) => {
    try {
        ...
        const stations = departments.flatMap((dep) => dep.lines.flatMap((line) => line.stations.map((s) => ({ ...s.toObject(), lineId: line._id, departmentId: dep._id }))));
        res.json(stations);
    }
    catch (err) {
        next(err);
...
export const createStation = async (req, res, next) => {
    try {
        ...
        if (!line)
            return res.status(404).json({ message: 'Line not found' });
        line.stations.push({ name, tenantId: req.tenantId });
...
export const getStationsByLine = async (req, res, next) => {
    try {
        const department = await Department.findOne({
