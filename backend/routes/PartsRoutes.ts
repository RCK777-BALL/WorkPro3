...
const loadPart = async (req: any) => {
  const tenantId = req.user?.tenantId;
  const id = req.auditId || req.params.id;
  if (!id) return null;
  return Part.findOne({ _id: id, tenantId }).lean();
};
...
router.post(
  '/',
  withAudit('Part', 'create', loadPart, async (req: any, res) => {
    const tenantId = req.user?.tenantId;
...
router.put(
  '/:id',
  withAudit('Part', 'update', loadPart, async (req: any, res) => {
    const tenantId = req.user?.tenantId;
...
router.delete(
  '/:id',
  withAudit('Part', 'delete', loadPart, async (req: any, res) => {
    const tenantId = req.user?.tenantId;
...
router.post(
  '/:id/adjust',
  withAudit('Part', 'adjust', loadPart, async (req: any, res) => {
    const tenantId = req.user?.tenantId;
