...
import { configureOAuth, getOAuthScope } from '../auth/oauth';
configureOIDC();
configureOAuth();
const router = Router();
...
router.get('/oauth/:provider', (req, res, next) => {
    ...
    passport.authenticate(provider, { scope: getOAuthScope(provider) })(req, res, next);
});
router.get('/oauth/:provider/callback', (req, res, next) => {
    ...
    passport.authenticate(provider, { session: false }, (err, user) => {
        ...
        const redirectUrl = `${frontend}?token=${token}&email=${encodeURIComponent(user.email)}`;
