...
router.get('/purchase-orders', requirePermission('inventory.read'), async (req, res, next) => {
  try {
...
router.post('/purchase-orders', requirePermission('inventory.purchase'), async (req, res, next) => {
  try {
    const scope = buildScope(req.tenantId, req.siteId);
    if (!scope) {
      sendResponse(res, null, 'Tenant ID required', 400);
...
router.get('/purchase-orders/export', requirePermission('inventory.read'), async (req, res, next) => {
  try {
...
router.post('/purchase-orders/:id/status', requirePermission('inventory.purchase'), async (req, res, next) => {
  try {
    const scope = buildScope(req.tenantId, req.siteId);
    if (!scope) {
      sendResponse(res, null, 'Tenant ID required', 400);
