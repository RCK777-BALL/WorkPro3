...
const serializePart = (
  ...
  siteId: part.siteId ? part.siteId.toString() : undefined,
  name: part.name,
...
export const listParts = async (context: InventoryContext): Promise<PartResponse[]> => {
  ...
  const parts = await PartModel.find(query).lean();
...
const serializePurchaseOrder = async (
  ...
  return {
    ...
    siteId: po.siteId ? po.siteId.toString() : undefined,
...
const buildCsvBuffer = (orders: PurchaseOrderResponse[]): Buffer => {
  const parser = new Json2csvParser({
    fields: [
      ...
      { label: 'Auto generated', value: (row) => (row.autoGenerated ? 'Yes' : 'No') },
      { label: 'Expected date', value: (row) => formatDate(row.expectedDate) },
      { label: 'Created at', value: (row) => formatDate(row.createdAt) },
      { label: 'Line items', value: (row) => row.items.length },
      {
        label: 'Total quantity',
        value: (row) => row.items.reduce((sum, item) => sum + item.quantity, 0),
      },
    ],
  });
...
export const listAlerts = async (context: InventoryContext): Promise<InventoryAlert[]> => {
  ...
  const parts = await PartModel.find(match).lean();
  const refs = await resolveReferenceMaps(parts);
