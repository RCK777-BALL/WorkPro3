...
export const listDefinitions = async (context: WorkflowContext): Promise<WorkflowDefinitionDocument[]> => {
...
export const listInstances = async (context: WorkflowContext): Promise<WorkflowInstanceDocument[]> => {
  const tenantId = toObjectId(context.tenantId);
  if (!tenantId) return [];
...
export const listSlaPolicies = async (context: WorkflowContext): Promise<SlaPolicyDocument[]> => {
  const tenantId = toObjectId(context.tenantId);
  if (!tenantId) return [];
