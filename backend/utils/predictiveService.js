...
function getModel() {
    return (process.env.PREDICTIVE_MODEL || config.predictive?.model || 'linear');
}
...
export async function predictForAsset(assetId, tenantId) {
    ...
    const series = Array.from(metricMap.entries()).map(([metric, values]) => ({ metric, values }));
    ...
    for (const { metric, values } of series) {
        ...
        if (probability > FAILURE_THRESHOLD && asset?.tenantId) {
            await Notification.create({
                ...
                message: `Asset ${assetId} predicted failure probability ${(probability * 100).toFixed(1)}% for metric ${metric}`,
            });
        }
...
export async function getPredictions(tenantId) {
...
export async function getPredictionTrend(assetId, metric, tenantId) {
    const trendDocs = await Prediction.find({ asset: assetId, metric, tenantId })
        .sort({ timestamp: 1 })
        .limit(50);
