...
- [ ] **Secrets defaults hardening**: Replace placeholder secrets in `.env.sample` with generated guidance and validation rules (minimum entropy, rotation guidance, required-in-prod flags), plus add build/startup checks that reject `changeme`-style values in production configuration.【F:.env.sample†L1-L120】
- [ ] **Production secret provisioning**: Define and document a secure secret management workflow for Kubernetes (external secret store, sealed secrets, or cloud provider integration), then populate `workpro-backend-secrets` and `workpro-frontend-secrets` manifests with references rather than empty strings; include rotation and environment bootstrapping steps.【F:k8s/secrets.yaml†L1-L46】
