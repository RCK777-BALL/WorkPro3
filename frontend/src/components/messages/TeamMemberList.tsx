...
const TeamMemberList = ({ members }: TeamMemberListProps) => {
  ...
  const grouped = sortedMembers.reduce<Record<string, TeamMember[]>>((acc, member) => {
  ...
  return (
    <Stack gap="md" className="mt-3">
      {sortedMembers.length > 0 && (
        <>
          {groupedEntries.map(([initial, group]) => (
            <div key={initial}>
              <Text className="px-2 text-xs font-semibold uppercase tracking-wide text-gray-500">
                {initial}
              </Text>
              <Stack gap="xs" mt="xs">
                {group.map((member) => (
